...
bool MapViewer::executeInput(std::string input)
{
	...
	std::regex walk_expr(
			"(go|move|walk)( )(right|top|up|down|left|(w|W)est|(e|E)ast|(s|S)outh|(n|N)orth)");
	...
	for(int row = 0; row < amountRows; row++)
	{
		...
		for(int col = 0; col < amountCols; col++)
		{
			if(row % 4 == 0) // lines between fields
			{
				...
				if((row / 4) * 10 + col == _selectedField
						|| ((row - 4) / 4) * 10 + col == _selectedField) output +=
						"\033[31m━━━━━\033[0m"; // this or the previous row
				else output += "─────";
				...
				if(col == amountCols - 1) // last col
				{
					if(row == 0)
					{
						if((row / 4) * 10 + col == _selectedField) output +=
								"\033[31m┓\033[0m";
						else output += "┐";
					}
					else if(row == amountRows - 1)
					{
						if(((row - 4) / 4) * 10 + col == _selectedField) output +=
								"\033[31m┛\033[0m";
						else output += "┘";
					}
					else
					{
						if((row / 4) * 10 + col == _selectedField) output +=
								"\033[31m┪\033[0m";
						else if(((row - 4) / 4) * 10 + col == _selectedField) output +=
								"\033[31m┩\033[0m";
						else output += "┤";
					}
				}
			...
			{
				if((row / 4) * 10 + col == _selectedField
						|| ((row / 4) * 10 + col - 1 == _selectedField
								&& _selectedField % 10 != 9)) output +=
						"\033[31m┃\033[0m"; // this or the previous row
				else output += "│";
